...
def test_register_user_with_nudge_preference(client: TestClient, db: Session) -> None:
    """Test signup with nudge_preference field."""
    username = random_email()
    password = random_lower_string()
    full_name = random_lower_string()
    data = {
    ...
    assert created_user["email"] == username
    assert created_user["full_name"] == full_name
    assert created_user.get("nudge_preference", "daily") == "weekly"

    user_query = select(User).where(User.email == username)
    user_db = db.exec(user_query).first()
    assert user_db
    assert user_db.nudge_preference == "weekly"
